        ...
        all_images[all_images.length-1].onload = function() {
            for(var i=0, len=all_images.length; i<len; i++){
                if (all_images[i].width>all_images[i].height)
                    all_context[i].drawImage(all_images[i], 0, 0, all_images[i].width, all_images[i].height, 0, 0, 205, 140);
                else{
                    all_context[i].drawImage(all_images[i], 0, 0, all_images[i].width, all_images[i].height, 0, 0, 205, 140);
                }
            }
...
//add element to #uploaded
    function displayElements(){
        ...
        for(var i=0, len=FILES_TO_SUBMIT.length; i<len; i++)
        {
            ...
            html_el += '<canvas id="pic_' + element_id + '"height="140" width="205"/>';
...
// add price after our fee
var $price = $("#id_price");
var fee = $price.closest("form").data("fee");
fee = parseInt(fee);

$price.keyup(function(){
    var price = parseInt($(this).val());
    var real_price = price + price*(fee/100);
    var $real_price = $(".real_price");
    $real_price.text(Math.ceil(real_price));
});
